...
class ProfilRespondenSurvei_model extends CI_Model
{
    ...
    var $column_search     = array('nama_profil_responden', 'nama_alias');
    ...
    {
        // $this->db->select('*');
        // $this->db->from("(SELECT '0' AS id, 'Nama Lengkap' AS nama_profil_responden, '' AS jenis_isian, '' AS is_default, '' AS type_data UNION SELECT '0' AS id, 'Provinsi Domisili' AS nama_profil_responden, '' AS jenis_isian, '' AS is_default, '' AS type_data UNION SELECT '0' AS id, 'Kota / Kabupaten Domisili' AS nama_profil_responden, '' AS jenis_isian, '' AS is_default, '' AS type_data UNION SELECT id, nama_profil_responden, jenis_isian, is_default, type_data FROM profil_responden_$table_identity) AS pro_res_$table_identity");
